...
object MyModule {
  ...
  // Q:  does n start from 1 or from 0?
  // I thought it started at 1 (the first)
  def fib(n: Int): Int =
  {
    def loop(n: Int, i: Int, one: Int, two: Int): Int =
    {
      if (n == 0) 0
      else if (i == n) two
      else loop(n, i+1, two, one+two)
    }
...
object PolymorphicFunctions {
  ...
  {
    def loop(i: Int, result: Boolean): Boolean =
    {
      if (i == 0 || i >= as.length) result
      else if (!result) result
      else loop(i+1, gt(as(i-1), as(i)))
    }
  ...
  def uncurry[A,B,C](f: A => B => C): (A, B) => C =
    // nope (a: A) => (b: B) => (a, b) => f(a)
    // nope (a: A, b: B) => (a, b) => f(a)
    // nope (a: A, b: B) => f(a)
    // nope (a: A, b: B) => f.uncurried
    // nope (a: A, b: B) => f(a, b)
    (a: A, b: B) => f(a)(b)
    // ??? I thought that f has one argument, a
    // no, f has two arguments, a and b
